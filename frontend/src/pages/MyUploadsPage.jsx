...
const MyUploadsPage = () => {
  ...
  const handleSearch = async (query) => {
    ...
    try {
      ...
      const userResults = docResponse.data.results.filter(doc => doc.uploaded_by_username === user.username);
                  ...
                  <HStack justify="space-between" mb={4}>
                    <HStack spacing={4}>
                      <Tooltip label="Upload document" placement="right">
                        <IconButton
                          icon={<FiPlus />}
                          onClick={() => navigate('/upload', { state: { from: '/my-uploads', folderId: currentFolderId } })}
